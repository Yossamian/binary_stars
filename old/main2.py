...
class StarDataset(Dataset):
    # load the dataset
    def __init__(self, labels, paths = list(range(10)), normalize=True):

        root_dir = Path("drive/Shareddrives/Learning_Deep/project_954437307_066610346/data")
        ...
        for num in paths:
          # Read file
          path=root_dir.joinpath(f'{num}.npz')
          data = np.load(path)
          len_batch=len(data['list_vsini_1'])
          list_batch=list((path, num) for num in range(len_batch))
          all_list = all_list + list_batch

        self.rows = np.array(all_list)
        self.labels=labels
    ...
    def __getitem__(self, idx):
      filename, index = self.rows[idx]
      data = np.load(filename)
      x = data['fi'][index]
      y = [data[lab][index] for lab in self.labels]
